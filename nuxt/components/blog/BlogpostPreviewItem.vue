<template>
  <NuxtLink
    :to="localePath({ name: 'blog-slug', params: { slug: post.slugWithoutLocale } })"
  ...
  >
    ...
    >
      <div class="mb-2">
        ...
        >
          <span
            ...
            :key="id"
            class="truncate tracking-wider font-medium text-ss px-2 py-1 rounded-full mr-2 mb-2 border border-light-border dark:border-dark-border transition-colors duration-300 ease-linear"
          >
            {{ tag }}
          </span>
        </div>
      ...
      <div class="flex flex-col lg:flex-row text-sm">
        <div v-if="post.authors" class="mb-4 lg:mb-0 flex items-center">
          <span
        ...
        >
          ...
          >
            {{ formatDateByLocale(post.createdAt) }}
          </span>
          <span
            class="hidden lg:inline-block text-xs mx-2 light:text-light-onSurfaceSecondary dark:text-dark-onSurfaceSecondary transition-colors duration-300 ease-linear"
          >
