<template>
  <div class="w-full relative flex flex-col justify-between">
    ...
    >
      ...
      >
        <NuxtLink
          :to="localePath({ name: 'slug', params: { slug: result.slug } })"
...
export default {
  ...
  watch: {
    async q (q) {
      ...
      this.results = await this.$content({ deep: true })
        .where({ locale: { $eq: this.$i18n.defaultLocale } })
        .sortBy('createdAt', 'desc')
        .only(['title', 'slug', 'category', 'path']).limit(12).search(q).fetch()
        if (this.$i18n.defaultLocale !== this.$i18n.locale) {
        try {
          const translatedPosts = await this.$content({ deep: true })
            .where({ locale: { $eq: this.$i18n.locale } })
            .sortBy('createdAt', 'desc')
            .fetch()
  ...
  methods: {
    ...
    go () {
      ...
      const ro = this.localePath({ name: 'slug', params: { slug: result.slug } })
      this.$router.push(this.localePath({ name: 'slug', params: { slug: result.slug } }))
